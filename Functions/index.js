    ...
    try {
        ...
        while (hasMore) {
            const { data: batch, error: passesError } = await supabase
                .from('passes')
                .select('id, player_id, x_start, y_start, xt')
                .eq('match_id', match_id)
                .gte('xt', 0)
        ...
        passes.forEach(pass => {
            ...
            let xtVal = pass.xt || 0;
            if (xtVal <= 0.00000001) xtVal = 0; // Ignorăm trucul și valorile minuscule
            ...
            if (pId) {
                ...
                playerMatrices[pId][sY][sX] = parseFloat((playerMatrices[pId][sY][sX] + xtVal).toFixed(6));
            }
            if (tId) {
                if (!teamMatrices[tId]) teamMatrices[tId] = createEmptyMatrix();
                teamMatrices[tId][sY][sX] = parseFloat((teamMatrices[tId][sY][sX] + xtVal).toFixed(6));
            }
        });
